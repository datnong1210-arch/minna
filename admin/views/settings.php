...
if (isset($_POST['ahominna_settings_submit']) && check_admin_referer('ahominna_settings_nonce')) {
    ...
    update_option('ahominna_vocab_per_page', intval($_POST['vocab_per_page']));
...
<div class="wrap ahominna-settings">
    ...
    <form method="post" class="ahominna-settings-form">
        ...
        <table class="form-table">
            ...
            <tr>
                <th scope="row">
                    <label for="vocab_per_page"><?php _e('Vocabulary Per Page', 'ahominna'); ?></label>
                </th>
                <td>
                    <input type="number" name="vocab_per_page" id="vocab_per_page" value="<?php echo esc_attr($vocab_per_page); ?>" min="10" max="500" />
                    <p class="description"><?php _e('Maximum number of vocabulary items to load per lesson (for performance)', 'ahominna'); ?></p>
                </td>
            </tr>
        </table>
